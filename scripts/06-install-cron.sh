...
echo "0 6 * * * cd $PROJECT_ROOT && $VENV_PYTHON -m src.services.weather >> $PROJECT_ROOT/logs/cron.log 2>&1" >> "$TMP_NEW"
echo "5 * * * * cd $PROJECT_ROOT && $VENV_PYTHON -m src.jobs.hourly_weather_sync >> $PROJECT_ROOT/logs/cron.log 2>&1" >> "$TMP_NEW"
echo "0 7 * * * cd $PROJECT_ROOT && $VENV_PYTHON -m src.jobs.morning_weather_report >> $PROJECT_ROOT/logs/cron.log 2>&1" >> "$TMP_NEW"
