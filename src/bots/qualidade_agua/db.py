#!/usr/bin/env python3
"""
Acesso ao PostgreSQL com asyncpg para o bot de qualidade da água.
"""
import os
import asyncpg
...
# Reuso de funções de lote para o bot de água (opcional se quiser gerir lotes por aqui também)
async def criar_lote_completo(dados: dict) -> str:
    pool = await get_pool()
    async with pool.acquire() as conn:
        await conn.execute(
            """
            INSERT INTO lotes (estrutura_uid, lote, data_alojamento, peixes_alojados, peso_medio, area_acude, densidade, descricao)
            VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
            """,
            dados['estrutura_uid'], dados['lote'], dados['data_alojamento'],
            dados['peixes_alojados'], dados['peso_medio'], dados['area_acude'], dados['densidade'], dados.get('descricao')
        )
    return dados['lote']
...
async def finalizar_lote_abate(dados: dict) -> bool:
    ...
    async with pool.acquire() as conn:
        result = await conn.execute(
            "UPDATE lotes SET data_abate = $1 WHERE estrutura_uid = $2 AND data_abate IS NULL",
            dados['data_abate'], dados['estrutura_uid']
        )
