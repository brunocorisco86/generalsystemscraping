...
LOG_FILE = os.path.join(os.environ.get("LOGS_DIR", "logs"), "bot_query_temp_15d.log")
logging.basicConfig(
    level=logging.INFO,
...
def get_fortnightly_temp_report():
    logger.info("Iniciando geração de relatório quinzenal de temperatura (15 dias).")
    now = datetime.now()
    ...
    try:
        ...
        if df.empty:
            ...
            send_telegram_message("ℹ️ Nenhum dado de temperatura encontrado nos últimos 15 dias.", chat_id=CHAT_ID_FROM_ARGS)
            return
        ...
        for tank, struct_df in df.groupby('nome_estrutura'):
            ...
            if not struct_df.empty:
                ...
                plt.plot(struct_df.index, struct_df['temperatura'], label=f'Bruto: {tank}', linewidth=0.8, alpha=0.5)

                # Média móvel da máxima e mínima diária
                daily_stats = struct_df['temperatura'].resample('D').agg(['min', 'max'])
        ...
        # Ajustes de Eixo
        v_min = df['temperatura'].min()
        v_max = df['temperatura'].max()
        plt.ylim(max(0, v_min - 2.0), v_max + 2.0)
        
        plt.title('Histórico de Temperatura (15 Dias) - Tendências Diárias')
        ...
        plot_path = os.path.join(REPORT_DIR, 'temp_15d_trend.png')
        ...
        send_telegram_message(f"❌ Erro crítico ao gerar relatório de temperatura (15 dias): {e}", chat_id=CHAT_ID_FROM_ARGS)
    finally:
        if conn:
            conn.close()

if __name__ == "__main__":
    get_fortnightly_temp_report()
